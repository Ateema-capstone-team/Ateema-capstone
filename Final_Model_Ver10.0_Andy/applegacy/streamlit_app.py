...
def _render_table(label: str, sel, all_products: set[str], prepay_full_year: bool, is_advertiser: bool, grand_total: float | None = None, include_award: bool = False): # Gabri Award# Dazhou 11/17 Advertiser # Yuchen 11/19 Early Bird # Yuchen 11/20 Networking Event
    import pandas as pd
...
if state:
    ...
    t_real_total = _render_table("tourist", t_sel, all_products, prepay_full_year, is_advertiser, grand_total, award_selected)
    st.markdown("---")
    st.markdown("---")

    st.subheader("Industry Pool")
    i_real_total = _render_table("industry", i_sel, all_products, prepay_full_year, is_advertiser, grand_total, award_selected)
    st.markdown("---")
