...
DEFAULT_PRODUCTS = r"C:\Users\Dazhou Wu\Desktop\Mine\master\Fall 25\Capstone ii\Model\Final_Model_Ver4.0_Andy\Data\PriceStrategy"
...
def _render_table(label: str, sel, all_products: set[str], prepay_full_year: bool, is_advertiser: bool, billing_date=None, grand_total: float | None = None,): # Dazhou 11/17 Advertiser # Yuchen 11/19 Early Bird # Yuchen 11/20 Networking Event
    ...
    if rows:
        st.write(f"**Subtotal (before discounts):** ${sel.subtotal:,.2f}") # Dazhou 11/24 grand total 
        df = pd.DataFrame(
            rows,
            columns=[
                "product",
...
if st.session_state.get("_trigger_generate"):
    ...
    st.markdown(f"### Grand Total: ${grand_total:,.2f} (Hard cap = ${total_budget*1.10:,.0f})")
